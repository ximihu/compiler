...
exception No_such_symbol
 ...
 let rec trans_stmt ast env =
   ...
   | Print e -> let (vl,_) = trans_exp e env in (print_int vl; print_string "\n"; env) 
   and trans_exp ast env =
     match ast with 
    | Id v -> ((env v),env) 
    | Num n -> (n,env)
    | Plus (e1,e2) -> let (vl1,_) = trans_exp e1 env in let (vl2,_) = trans_exp e2 env in (vl1 + vl2,env) 
    | Minus (e1,e2) -> let (vl1,_) = trans_exp e1 env in let (vl2,_) = trans_exp e2 env in (vl1 - vl2,env)  
    | Times (e1,e2) -> let (vl1,_) = trans_exp e1 env in let (vl2,_) = trans_exp e2 env in (vl1 * vl2,env) 
    | Div (e1,e2) -> let (vl1,_) = trans_exp e1 env in let (vl2,_) = trans_exp e2 env in (vl1 / vl2,env)   
    | StmtExp (s, e) -> let env' =trans_stmt s env in let (vl,_)= trans_exp e env' in (vl,env')
